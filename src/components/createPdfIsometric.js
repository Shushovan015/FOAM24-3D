...
export const createPdfIso = (foam, shapesArray, shapeToGeom2, opts = {}) => {
  ...
  const loopsFromGeom2 = (geom2) => {
    ...
    for (const s of adj.keys()) {
      for (const n of adj.get(s)) {
        const e0 = `${s}->${n}`; if (visited.has(e0)) continue;
        const loop = [un(s)]; let prev = s, cur = n; visited.add(e0);
        while (cur !== s) {
          loop.push(un(cur));
          ...
          if (nbr.length > 2) {
            ...
            for (const cand of nbr) {
              ...
              if (lt < bestAng) { bestAng = lt; best = cand; }
            }
            if (best) pick = best;
          }
          const e1 = `${cur}->${pick}`; if (visited.has(e1)) break;
          visited.add(e1); prev = cur; cur = pick;
        }
        if (loop.length >= 3) {
          const A = loop.reduce((s, p, i) => { const q = loop[(i + 1) % loop.length]; return s + (p[0] * q[1] - p[1] * q[0]); }, 0);
          if (A < 0) loop.reverse();
          loops.push(loop);
        }
      }
    }
    const area = (L) => Math.abs(L.reduce((s, p, i) => { const q = L[(i + 1) % L.length]; return s + (p[0] * q[1] - p[1] * q[0]); }, 0));
  ...
  const projIso   = ([x, y, z]) => { const a = Math.PI / 6; return [(x - y) * Math.cos(a), (x + y) * Math.sin(a) - z]; };
  ...
  const line = (page, a, b, th) => page.drawLine({ start: { x: a[0], y: a[1] }, end: { x: b[0], y: b[1] }, thickness: th });
  ...
  document.querySelector("#pdf-iso-button").onclick = async () => {
    ...
    page.drawRectangle({ x: fx, y: fy, width: fw, height: fh, borderWidth: FRAME.thick, color: PDF.rgb(1, 1, 1) });
    ...
    if (cfg.originMode === "min") {
      ...
      if (cMin === cCtr) {
        ...
        const dMin = Math.hypot(centerShapes[0] - (candidateMin.minX + candidateMin.maxX) / 2,
                                centerShapes[1] - (candidateMin.minY + candidateMin.maxY) / 2);
        const dCtr = Math.hypot(centerShapes[0] - (candidateCenter.minX + candidateCenter.maxX) / 2,
    ...
    const drawTop = () => {
      ...
      for (const p of pockets) {
        ...
        page.drawText(lbl, { x: c[0] - font.widthOfTextAtSize(lbl, cfg.fontSize) / 2, y: c[1] - cfg.fontSize / 2, size: cfg.fontSize, font });
      }
    ...
    const drawFront = () => {
      const all = [[foamRect.minX, 0], [foamRect.maxX, foam.sizeZ]];
      const fit = fit2rect(all, boxFront, 18);
      const T = (p) => fit.transform(p);

      drawLoop(page, [
        [foamRect.minX, 0], [foamRect.maxX, 0],
        [foamRect.maxX, foam.sizeZ], [foamRect.minX, foam.sizeZ],
      ].map(T), cfg.stroke);

      for (const p of pockets) {
        const r = [
          [p.bb.minX, 0], [p.bb.maxX, 0],
          [p.bb.maxX, p.depth], [p.bb.minX, p.depth]
        ].map(T);
    ...
    const drawSide = () => {
      const all = [[foamRect.minY, 0], [foamRect.maxY, foam.sizeZ]];
      const fit = fit2rect(all, boxSide, 18);
      const T = (p) => fit.transform(p);

      drawLoop(page, [
        [foamRect.minY, 0], [foamRect.maxY, 0],
        [foamRect.maxY, foam.sizeZ], [foamRect.minY, foam.sizeZ],
      ].map(T), cfg.stroke);

      for (const p of pockets) {
        const r = [
          [p.bb.minY, 0], [p.bb.maxY, 0],
          [p.bb.maxY, p.depth], [p.bb.minY, p.depth]
        ].map(T);
        drawLoop(page, r, cfg.stroke);
        if (cfg.showDepthInside && p.depth > 0) {
          const mid = [(r[0][0] + r[1][0]) / 2, (r[1][1] + r[2][1]) / 2];
