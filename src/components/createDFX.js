export const createDFX = (
  ...
  btn.onclick = async () => {
    // --- 1) Build all 2D outlines via shapeToGeom2 ---
    // foam border
    ...
    const others = shapesArray.map(shape => {
      ...
      // flatten to a simple array of vertices:
    ...
    // --- 2) Compute real extents from foamPts (so viewer auto-zooms correctly) ---
    const xs = foamPts.map(p=>p[0]), ys = foamPts.map(p=>p[1]);
    const minX = Math.min(...xs), maxX = Math.max(...xs);
    const minY = Math.min(...ys), maxY = Math.max(...ys);

    // --- 3) Build DXF text lines ---
    ...
    // ENTITIES: first the foam border
